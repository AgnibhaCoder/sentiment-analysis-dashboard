...
from app.model import get_toxicity_prediction
from app.tasks import process_batch_toxicity

app = FastAPI(title="Toxicity Classification Microservice")
...
async def analyze_toxicity(request: TextRequest):
    ...
    prediction = get_toxicity_prediction(processed_text)
...
async def trigger_batch_toxicity(request: BatchTextRequest):
    if not request.texts:
        raise HTTPException(status_code=400, detail="Text array cannot be empty")
        
    task = process_batch_toxicity.delay(request.texts)
    return {
        "job_id": task.id,
        "message": f"Toxicity batch job dispatched for {len(request.texts)} items."
    }
